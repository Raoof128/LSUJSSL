...
def main():
    ...
    run_command("python -m satellite_sim.cli.sat_cli send --station legit --cmd 'UPDATE_ORBIT_PARAMETERS'")
    run_command("python -m satellite_sim.cli.sat_cli send --station legit --cmd 'DEPLOY_SOLAR_PANEL'")
    
    # Scenario 2: Attack Attempts
    print("\n\n⚠️  SCENARIO 2: ROGUE STATION ATTACK ATTEMPTS")
    print("-" * 80)
    run_command("python -m satellite_sim.cli.sat_cli send --station rogue --cmd 'SHUTDOWN_REACTOR' --attack-type BAD_SIGNATURE")
    run_command("python -m satellite_sim.cli.sat_cli send --station rogue --cmd 'DISABLE_ATTITUDE_CONTROL' --attack-type NO_SIGNATURE")
    run_command("python -m satellite_sim.cli.sat_cli send --station rogue --cmd 'CHANGE_ORBIT' --attack-type SHORT_SIGNATURE")
